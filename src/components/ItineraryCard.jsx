...
export default function ItineraryCard({ place }) {
  return (
    <div className="bg-white p-3 rounded-xl shadow-md flex flex-col md:flex-row gap-4 items-start w-full">
      
      {/* Image */}
      <div className="w-full md:w-[112px] h-[180px] md:h-[112px] flex-shrink-0">
        <img
          src={place.image}
          alt={place.title}
          className="rounded-md w-full h-full object-cover"
      ...
      <div className="flex flex-col flex-grow w-full">
        ...
        {/* Description block */}
        <div className="flex w-full bg-gray-100 items-start rounded-md p-2">
          <p className="text-sm text-gray-700 flex-grow">
            {place.description}
          </p>
          <Edit2 className="w-4 h-4 mt-1 ml-2 hover:text-green-600" />
        </div>
      </div>
    </div>
  );
